...
.controller('playlistController', function($scope, $rootScope, $location, $window, Playlist, $sce, $http) {
   ...
   $http ({
    ...
    .then(function (resp) {
      var formatDate = function(str){
          ...
          var result = '';
          result += str[0]+' ';
          result += month_names[+str[1] - 1] + ' ';
          result += str[2] + ' ';
          result += str[3].slice(0, 5);
          return result;
      };
      ...
      // $rootScope.songs is for search
        ...
        $scope.eventName = resp.data.eventName;
        $scope.startTime = formatDate(resp.data.startTime);
        $scope.endTime = formatDate(resp.data.endTime);
 ...
 $scope.fbShare = $sce.trustAsHtml('<button class="btn btn-success">Invite Friends on Facebook</button> ');
